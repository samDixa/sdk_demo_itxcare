...
//    private static final String SERIAL_PORT_PATH = "/dev/ttyS4";
                ...
                if (inputStream != null) {
                    ...
                    if (bytesRead == STRUCT_SIZE) {
                        ...
                        synchronized (packetLock) {
                            ...
                            checkCameraChange(newPacket); // Added Camera check
    ...
    private void cameraOneOnMethod() {
        try {
            long startTime = System.currentTimeMillis();
            GlobalVars.setCam1On(true); // Front camera ON
            GlobalVars.setCam2On(false); // USB camera OFF
            mainHandler.post(() -> {
                Toast.makeText(getApplicationContext(), "Camera 1 (Front) Activated", Toast.LENGTH_SHORT).show();
            });
            Log.d(TAG, "Camera 1 turned ON");
            long endTime = System.currentTimeMillis();
        } catch (Exception e) {
            Log.e(TAG, "Failed to turn CAMERA ONE ON: " + e.getMessage() + " at " + System.currentTimeMillis(), e);
        }
    }

    private void cameraTwoOnMethod() {
        try {
            long startTime = System.currentTimeMillis();
            GlobalVars.setCam2On(true); // USB camera ON
            GlobalVars.setCam1On(false); // Front camera OFF
            mainHandler.post(() -> {
                Toast.makeText(getApplicationContext(), "Camera 2 (USB) Activated", Toast.LENGTH_SHORT).show();
            });
            Log.d(TAG, "Camera 2 turned ON");
            long endTime = System.currentTimeMillis();
        } catch (Exception e) {
            Log.e(TAG, "Failed to turn CAMERA TWO ON: " + e.getMessage() + " at " + System.currentTimeMillis(), e);
        }
    }

    private void checkCameraChange(CPUData newPacket) {
        if (newPacket == null) {
            return;
        }
        if (previousPacket == null) {
            Log.d(TAG, "Initial CAMERA Packet: ONE=" + newPacket.ONE + ", TWO=" + newPacket.TWO);
            if (newPacket.ONE == 1 && newPacket.TWO == 0) {
                cameraOneOnMethod();
            } else if (newPacket.TWO == 1 && newPacket.ONE == 0) {
                cameraTwoOnMethod();
            }
            return;
        }

        if (previousPacket.ONE != newPacket.ONE || previousPacket.TWO != newPacket.TWO) {
            if (newPacket.ONE == 1 && newPacket.TWO == 0) {
                cameraOneOnMethod();
            } else if (newPacket.TWO == 1 && newPacket.ONE == 0) {
                cameraTwoOnMethod();
            } else {
                Log.d(TAG, "Invalid CAMERA state: ONE=" + newPacket.ONE + ", TWO=" + newPacket.TWO);
            }
        } else {
            Log.d(TAG, "No CAMERA state change detected");
        }
    }
...
//            long endTime = System.currentTimeMillis();
//        } catch (Exception e) {
//            Log.e(TAG, "Failed to turn Camera 1 ON: " + e.getMessage() + " at " + System.currentTimeMillis(), e);
//        }
//    }
//
//    private void cameraTwoOnMethod() {
...
//            long endTime = System.currentTimeMillis();
//        } catch (Exception e) {
//            Log.e(TAG, "Failed to turn Camera 2 ON: " + e.getMessage() + " at " + System.currentTimeMillis(), e);
//        }
//    }
//
//    private void checkCameraChange(CPUData newPacket) {
